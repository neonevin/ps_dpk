...
# V 1.0 - This for app servers ONLY
# ***************************************************************
class my_cfg_roles::my_appserver{

  notify { 'Applying my_cfg_roles::my_appserver': }
  ...
  if $env_type != 'midtier' { # try midtier first - Might need to new classification webtier if doesnt work while testing 
    fail('The my_appserver role can only be applied to env_type of midtier')
  }
  contain ::my_cfg_profiles::my_appdom_cfg
