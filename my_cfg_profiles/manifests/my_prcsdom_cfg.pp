# ***************************************************************
# UQ pupppet profile for managing process scheduler  domain config
# UQ - Nevin Prasannan - 25/10/2018
# V 1.0 - This for process schedulersONLY
# ***************************************************************
class my_cfg_profiles::my_prcsdom_cfg{

  notify { 'Applying my_cfg_profiles::my_prcsdom_cfg': }
  ...
  if ($psft_single_installer) and ($psft_single_installer != '') {
    ...
    $prcsserver_domain_list = hiera('prcs_domain_list') # List of all the app server domains
    #NP  - Begin loop for domain list
    $prcsserver_domain_list.each |$domain_name, $prcsserver_domain_info| {
      notify {"Checking Config for PrcsServer domain ${domain_name}":}

      $db_settings        = $prcsserver_domain_info['db_settings']
      validate_hash($db_settings)
      # $db_settings_array  = join_keys_to_values($db_settings, '=')
      # notify {"PrcsServer domain ${domain_name} with the Database settings\n":}

      $config_settings    = $prcsserver_domain_info['config_settings']
      validate_hash($config_settings)
      # $config_settings_array = join_keys_to_values($config_settings, '=')
      # notify {"PrcsServer domain ${domain_name} Config settings: [${config_settings_array}]\n":}

      # $feature_settings   = $prcsserver_domain_info['feature_settings']
      # validate_hash($feature_settings)
      # $feature_settings_array = join_keys_to_values($feature_settings, '=')
      # notify {"PrcsServer domain ${domain_name} Feature settings: ${feature_settings_array}\n":}
      ...
      # $env_settings   = $prcsserver_domain_info['env_settings']
      # if $env_settings {
      #   validate_hash($env_settings)
      #   $env_settings_array = join_keys_to_values($env_settings, '=')
      #   notify {"PrcsServer domain ${domain_name} Env settings: ${env_settings_array}\n":}
      # }
      # get the database platform
      # $db_platform       = upcase($db_settings['db_type'])

      # Manage env config script 
      if (!defined(::my_cfg_man::my_Envcfg_Man["prcs_${domain_name}"])){
        @::my_cfg_man::my_envcfg_man{ "prcs_${domain_name}":
          ensure             => present,
          dom_settings_array => $prcsserver_domain_info,
          domain_name        => $domain_name,
          domain_type        => 'prcs',
          cfg_log_file       => $log_path,
        }
      }
      realize(::my_cfg_man::my_Envcfg_Man["prcs_${domain_name}"])

      $prcsserver_cfg_file = "${prcsserver_domain_info['ps_cfg_home_dir']}/appserv/prcs/${domain_name}/psprcs.cfg"
      if (!defined(::my_cfg_man::my_Tuxdomcfg_Man["prcs_${domain_name}"])){
        @::my_cfg_man::my_tuxdomcfg_man{ "prcs_${domain_name}":
          ensure              => present,
          cfg_file            => $prcsserver_cfg_file,
      ...
      realize(::my_cfg_man::my_Tuxdomcfg_Man["prcs_${domain_name}"])
    }
